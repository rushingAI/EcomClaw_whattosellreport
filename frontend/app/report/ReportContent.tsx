...
export default function ReportContent({ keyword, accessToken }: ReportContentProps) {
  const router = useRouter();
  const marketplace = "us";
  const { state, start, rerun, abort } = useAnalysis();
  ...
  useEffect(() => {
    ...
    start(keyword, marketplace, accessToken);
    return () => {
      abort();
    };
  }, [keyword, accessToken]); // eslint-disable-line react-hooks/exhaustive-deps
